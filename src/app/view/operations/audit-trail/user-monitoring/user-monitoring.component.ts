  ...
  ExportPdfClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      ...
      if (this.sendCustomData?.length > 0) {
        this.slaservice.exportAsPdfFileLogs(this.sendCustomData, data, 'User')
      }
      else {
  ...
  ExportExcelClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      res.result.forEach((item: any) => {
        this.listkeys = ["S.no", "Action Performed By", "Log Action", "User Name", "Action Date"];
      ...
      if (this.sendCustomDataExcel?.length > 0) {
        this.slaservice.exportExcelFileLogs(this.sendCustomDataExcel, this.listkeys, data, 'User')
      }
      else {
        this.service.sweetAlertMsg('error', 'Data not Found');
        return;
