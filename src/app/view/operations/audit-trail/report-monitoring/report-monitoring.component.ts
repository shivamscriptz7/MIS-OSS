...
export class ReportMonitoringComponent implements OnInit {
  ...
  public getReportLogDetails: Subscription | any;
  public search: string = '';
  public otherParams: any;
  public reportLogList: any = [];
  public logData: any;
  public moduleId: any;
  public oldValue: any;
  public newValue: any;
  ...
  getUserReports() {
    ...
    this.dtOptions = {
      ...this.commonService.settingDataTableServer(), ajax: (dataTablesParameters: any, callback, settings) => {
        ...
        this.getReportLogDetails = this.service.getAPIMethod(url + '?' + params).pipe(debounceTime(500)).subscribe((resp) => {
          ...
          if (resp.result) {
            if (resp.result) {
              this.reportLogList = resp.result.map((data: any) => {
  ...
  ExportPdfClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      ...
      if (this.sendCustomData?.length > 0) {
        this.slaservice.exportAsPdfFileLogs(this.sendCustomData, data, "Report")
      }
      else {
  ...
  ExportExcelClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      res.result.forEach((item: any) => {
        this.listkeys = ["S.no", "Action Performed By", "Log Action", "Report Name", "Action Date"];
      ...
      if (this.sendCustomDataExcel?.length > 0) {
        this.slaservice.exportExcelFileLogs(this.sendCustomDataExcel, this.listkeys, data, 'Report')
      }
      else {
        this.service.sweetAlertMsg('error', 'Data not Found');
        return;
