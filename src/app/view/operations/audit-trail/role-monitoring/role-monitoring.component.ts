...
@Component({
  selector: 'app-role-monitoring',
  templateUrl: './role-monitoring.component.html',
  styleUrls: ['./role-monitoring.component.scss']
})
export class RoleMonitoringComponent implements OnInit {
  ...
  ExportExcelClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      res.result.forEach((item: any) => {
        this.listkeys = ["S.no", "Action Performed By", "Log Action", "Role Name", "Action Date"];
      ...
      if (this.sendCustomDataExcel?.length > 0) {
        this.slaservice.exportExcelFileLogs(this.sendCustomDataExcel, this.listkeys, data, 'Role')
      }
      else {
        this.service.sweetAlertMsg('error', 'Data not Found');
        return;
