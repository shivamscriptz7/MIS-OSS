...
@Component({
  selector: 'app-email-config-monitoring',
  templateUrl: './email-config-monitoring.component.html',
  styleUrls: ['./email-config-monitoring.component.scss']
})
export class EmailConfigMonitoringComponent implements OnInit {
  ...
  public getRoleLogDetails: Subscription | any;
  ...
  getUserRole() {
    ...
    this.dtOptions = {
      ...this.commonService.settingDataTableServer(), ajax: (dataTablesParameters: any, callback, settings) => {
        ...
        this.getRoleLogDetails = this.service.getAPIMethod(url + '?' + params).pipe(debounceTime(500)).subscribe((resp) => {
          this.loaderdata = false;
          this.ngxLoader.stop();
          if (resp.result) {
            if (resp.result) {
              this.roleList = resp.result.map((data: any) => {
  ...
  dialogBox(item: any) {
    if (item.LOG_DEL_REASON == null || item.LOG_DEL_REASON == '') {
      this.logOldValues = JSON.parse(item.LOG_OLD_VALUE);
    }
    else {
      this.logOldValues = JSON.parse(item.LOG_OLD_VALUE);
      ...
      item.LOG_OLD_VALUE = JSON.stringify(this.logOldValues);
    }
  ...
  ExportPdfClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      ...
      if (this.sendCustomData?.length > 0) {
        this.slaservice.exportAsPdfFileLogs(this.sendCustomData, data, "Email-Configuration")
      }
      else {
      ...
      //this.slaservice.exportAsPdfFileLogs(this.sendCustomData,data, 'Role')
    });
  }
  ...
  ExportExcelClick(data: any) {
    ...
    this.service.getAPIMethod(url).subscribe(res => {
      res.result.forEach((item: any) => {
        this.listkeys = ["S.no", "Action Performed By", "Log Action", "Group Name", "Action Date"];
      ...
      if (this.sendCustomDataExcel?.length > 0) {
        this.slaservice.exportExcelFileLogs(this.sendCustomDataExcel, this.listkeys, data, 'Email-Configuration')
      }
      else {
        this.service.sweetAlertMsg('error', 'Data not Found');
        return;
